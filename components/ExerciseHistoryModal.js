// components/ExerciseHistoryModal.js — exercise-specific session history modal
...
export default function ExerciseHistoryModal({ isOpen, onClose, exerciseName, logs }) {
  ...
  // Sort newest first
  const sorted = [...filtered].sort((a, b) => new Date(b.performed_at) - new Date(a.performed_at));
    ...
    >
      <div className={styles.modal}>
        ...
        <div className={styles.list}>
          {sorted.length === 0 && <p className={styles.empty}>No sessions found.</p>}
          {sorted.map((log) => (
            <div key={log.id} className={styles.item}>
              ...
              {log.notes && <div className={styles['item-notes']}>{log.notes}</div>}
            </div>
          ))}
        </div>
      </div>
