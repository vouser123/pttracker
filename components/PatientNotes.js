 ...
 *                               isConcerning, displayParts } — already filtered and sliced
  ...
  return (
    <div className={`${styles['notes-alert-section']} ${collapsed ? styles.collapsed : ''}`}>
      <div className={styles['notes-header']} onPointerUp={onToggle}>
        ...
        <span className={styles['notes-header-hint']}>
          tap to {collapsed ? 'expand' : 'collapse'}
        </span>
      </div>
      <div className={styles['notes-list']}>
        {notes.map((log) => (
          <div
            key={log.id}
            className={`${styles['note-card']} ${log.isConcerning ? styles.concerning : ''}`}
          >
            ...
            <div className={styles['note-text']}>
              {log.displayParts.map((part) => (
                <span
                  key={`${log.id}-${part.key}`}
                  className={part.isHighlighted ? styles['concerning-word'] : undefined}
                >
                  {part.text}
                </span>
              ))}
            ...
            <button
              type="button"
              className={styles['note-dismiss-btn']}
              onPointerUp={() => onDismiss(log.id)}
