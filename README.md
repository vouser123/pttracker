...
- [`app/(protected)/page.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/app/(protected)/page.js) + [`app/(protected)/TrackerPage.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/app/(protected)/TrackerPage.js) — tracker (session logging, history, messages). `page.js` is the protected App Router server entry and metadata owner; `TrackerPage.js` is the current client route orchestrator. Legacy parity baseline: `public/index.html` (on `static` branch / `legacy.pttracker.app`).
  - [`app/(protected)/TrackerRouteShell.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/app/(protected)/TrackerRouteShell.js) owns the render-only tracker shell: header, online/offline glyph (`🛜` when online, `🚫` when offline), banners, tab body, and bottom nav.
  - [`app/(protected)/TrackerOverlays.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/app/(protected)/TrackerOverlays.js) owns the render-only overlay stack for session logging, timer, next-set confirmation, session notes, messages, and dosage editing.
...
- [`biome.json`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/biome.json): Lightweight repo formatter/linter config. Use `npm run biome:check`, `npm run biome:lint`, or `npm run biome:format` manually when you want repo-wide tooling help. Repo-wide scans now exclude the reviewed auth pair [`hooks/useAuth.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/hooks/useAuth.js) and [`lib/auth.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/auth.js) so those files do not keep resurfacing in the shared backlog. For agent-friendly repo-wide analysis artifacts, use `npm run biome:report` to write a structured report under `C:\Users\cindi\OneDrive\Documents\PT_Backup\biome\logs`, then `npm run biome:report:summary` to print and save a compact triage summary from the newest JSON report.
...
- [`scripts/biome-report.mjs`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/scripts/biome-report.mjs): Repo-wide Biome report helper for Codex and Claude Code. It creates the shared backup log folder if needed, runs `biome check` with a structured reporter, and writes both a timestamped artifact and a `biome-check-latest.*` alias outside the repo so the output does not become part of the next Biome run.
- [`scripts/biome-report-summary.mjs`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/scripts/biome-report-summary.mjs): Reads the newest shared Biome JSON report (or a provided report path), then prints and saves a compact summary grouped by file, rule, and severity so agents can triage the backlog faster.
- [`scripts/commit-preflight.mjs`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/scripts/commit-preflight.mjs): Agent-facing commit preflight helper. Run `npm run commit:preflight -- --message "Your title (pt-xxxx)" --trailer "Co-Authored-By: Codex GPT-5.4 <codex@openai.com>"` after staging to surface README, Biome, structure, gitleaks, and commit-message blockers in one report before `git commit`.
...
- Biome pre-commit behavior: the hook auto-formats tracked commit files, re-stages them, prints which files it rewrote, then runs lint on the full staged set. A narrow approved bypass still exists for emergencies: set `PT_BIOME_SKIP_AUTOWRITE=1`, `PT_BIOME_SKIP_AUTOWRITE_APPROVED=1`, `PT_BIOME_SKIP_AUTOWRITE_REASON="..."`, and `PT_BIOME_SKIP_AUTOWRITE_FILE=path/to/file.js` before `git commit` to skip automatic formatting for exactly one staged file while the rest of the staged files still auto-format normally. Agents must use that bypass only after consulting the user and receiving explicit approval.
- Structure pre-commit behavior: the hook runs `node scripts/check-structure.mjs --staged` after Biome. It blocks commits when staged files violate machine-checkable structure rules from `docs/NEXTJS_CODE_STRUCTURE.md` and `docs/SYSTEM_ARCHITECTURE.md`. Agents must not bypass file-size caps or related structure rules without consulting the user and receiving explicit approval.
- Commit preflight: after staging, run `npm run commit:preflight -- --message "Your title (pt-xxxx)" --trailer "Co-Authored-By: Codex GPT-5.4 <codex@openai.com>"` to see README, Biome, structure, gitleaks, and commit-message blockers in one report before `git commit`.
...
- [`components/SessionLoggerSetCard.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/components/SessionLoggerSetCard.js): Repeated set-card renderer for `SessionLoggerModal`, including set fields and form-parameter rows. Use it when session-logger set-row UI changes without widening the modal shell.
...
- [`components/MessagesModal.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/components/MessagesModal.js): Messaging modal shell used by index and pt-view. Owns compose/send, open-state side effects, unread-clear handoff, archived-toggle state, and undo-send confirmation. Requires `onSend`, `onArchive`, `onUnarchive`, `onRemove`, `onMarkRead`, `onEmailToggle`, `onOpened` props from `useMessages`.
- [`components/MessagesThreadList.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/components/MessagesThreadList.js): Thread renderer for `MessagesModal`. Owns active bubble rows, rolled-up previews, archived rows, sender/timestamp labeling, and action buttons without widening the modal shell.
...
- [`components/ExerciseRolesWorkspace.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/components/ExerciseRolesWorkspace.js): Roles workspace shell with search/select controls and handoff into `ProgramRolesSection`. Use it when a route such as rehab coverage or the editor needs the full roles workspace, not just the add/remove table. Route hosts should pass precomputed exercise select options and role option lists so the component stays UI-only.
- [`components/ProgramRolesSection.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/components/ProgramRolesSection.js): Standalone roles-management section for the `/program` editor page. Use it when the exercise editor needs roles outside the core exercise form. The section now expects UI-ready region/capacity/focus/contribution options from the route host instead of shaping vocab rows itself.
- [`components/ProgramDosageWorkspace.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/components/ProgramDosageWorkspace.js): Dosage workspace shell with patient-context banner, search/select controls, and dosage summary card. Use it when an editor host needs the full dosage workspace while leaving modal editing to `DosageModal`. Route hosts should pass grouped exercise select options and the already-formatted dosage summary so the component remains presentational.
...
- [`components/ExerciseFormTagSection.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/components/ExerciseFormTagSection.js): Reusable select-plus-chip entry control used by `ExerciseFormCore` for equipment, muscle, and form-parameter tag lists. Use it when exercise-form sections need the same add/remove tag UI without pushing generic input mechanics back into the section renderer.
...
- [`lib/fetch-with-offline.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/fetch-with-offline.js): Shared authenticated JSON read wrapper for offline-aware route bootstrap and related reads. Use it when a loader needs typed offline, HTTP, and parse failures plus shared request success/failure evidence without moving route-specific fallback or copy into `lib/`.
...
- [`lib/rehab-coverage-constants.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/rehab-coverage-constants.js): Coverage formula thresholds and weights (`COVERAGE_CONSTANTS`). Edit here to adjust formula behavior; do not hardcode values elsewhere.
- [`lib/rehab-coverage-math.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/rehab-coverage-math.js): Shared math helpers used across the coverage signal files: `average`, `weightedAverage`, `groupExercisesByFocus`.
- [`lib/rehab-coverage-percent.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/rehab-coverage-percent.js): Formula 1 — 7-day density percentage (`calculatePercent`). Handles HIGH/MEDIUM contribution weighting and multi-focus balance.
- [`lib/rehab-coverage-color.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/rehab-coverage-color.js): Formula 2 — recency color score and RGB mapping (`calculateColorScore`, `colorScoreToRGB`). Higher score = greener = more recent.
- [`lib/rehab-coverage-opacity.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/rehab-coverage-opacity.js): Formula 3 — 21-day trend opacity (`calculateOpacity`). Slow decay, fast recovery via recent activity floors.
- [`lib/rehab-coverage-region.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/rehab-coverage-region.js): Region bar aggregation (`calculateRegionBar`). Weighted average of capacity bars, weight = HIGH-exercise count^1.3.
- [`lib/rehab-coverage-builder.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/rehab-coverage-builder.js): Data builder (`buildCoverageData`). Converts raw `/api/logs` + `/api/roles` responses to the full coverage data structure. Pure — no DOM, no fetch.
...
- [`lib/manual-log-state.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/manual-log-state.js): Pure manual-log draft-set helpers for seed-state creation, add/remove/patch behavior, form-data updates, validation, and normalization prep. Use it for deterministic manual-log set shaping instead of rebuilding those rules inside tracker hooks.
...
- [`lib/tracker-exercise-context.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/tracker-exercise-context.js): Pure tracker form-context enrichment for selected exercises, including side-aware default form data and historical parameter lookup from logs plus current draft-session sets.
...
- [`lib/network-status.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/network-status.js): Shared browser-and-request connectivity state for offline-aware app flows. Use it to classify expected network-unavailable failures, record recent request success/failure evidence, and read the current effective-online status without scattering raw `navigator.onLine` checks through auth, queue, or bootstrap code.
- [`lib/pt-view.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/pt-view.js): Page-domain helpers and fetch logic for the history dashboard. It stays focused on pt-view logs/programs reads, routine filtering, and history transforms. Use [`lib/messages.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/messages.js) for shared messaging API and badge-count helpers, and use [`lib/users.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/users.js) for shared user lookup, patient-context resolution, and email helpers.
- [`lib/messages.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/messages.js): Shared messaging API and badge-count helpers for migrated message flows. Use it from [`hooks/useMessages.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/hooks/useMessages.js) instead of mixing message transport into page-domain libs.
- [`lib/tracker-bootstrap.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/tracker-bootstrap.js): Shared tracker bootstrap load, cache read/write, and fallback-shaping helpers. Use it when tracker bootstrap persistence, cache restore, initial bootstrap fetch behavior, or bootstrap error copy changes instead of growing the tracker route hooks.
- [`lib/program-page-data.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/program-page-data.js): Pure `/program` cached-bootstrap shaping helpers. Use it for editor fallback state and cached snapshot structure, not page lifecycle decisions.
- [`lib/program-bootstrap-warmup.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/program-bootstrap-warmup.js): Shared `/program` warm-fill helper for cache inspection and missing-bootstrap writes. Use it when protected-route warmup rules change instead of broadening [`hooks/useProgramBootstrapWarmup.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/hooks/useProgramBootstrapWarmup.js).
- [`lib/pt-editor.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/pt-editor.js): Page-domain fetch and mutation helpers for the exercise editor, including controlled-vocab CRUD wrappers used by `/program`. Its authenticated read helpers now run through the shared offline-aware fetch wrapper, while `/program` still owns the network-or-cache bootstrap flow and route-specific fallback copy.
...
- [`hooks/useEffectiveConnectivity.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/hooks/useEffectiveConnectivity.js): React subscription hook for the shared effective-online detector. Use it when a component or hook needs the current browser online state plus recent request-evidence state without re-subscribing to `window` events locally.
- [`hooks/usePtViewData.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/hooks/usePtViewData.js): Loads and caches the `/pt-view` bootstrap data for users, programs, and logs, including patient-context resolution, sign-out cleanup, and offline fallback. It now preserves the last visible cached snapshot when auth is still present but patient context has not re-resolved yet, uses the shared offline-aware read wrapper through its loader helpers, and distinguishes offline fallback from non-offline live-refresh failure in its route-level notice copy. Use it to keep the history dashboard route at orchestrator level instead of embedding fetch and cache lifecycle logic in the page.
...
- [`hooks/useRehabCoverageData.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/hooks/useRehabCoverageData.js): Loads rehab coverage logs and roles, renders cached coverage data immediately when IndexedDB already has a usable snapshot, writes refreshed rehab cache data, and handles offline fallback/reload for `/rehab`. It now uses the shared offline-aware read wrapper and distinguishes offline fallback from non-offline live-refresh failure in its route-level notice copy. Use it for coverage bootstrap work instead of putting route fetch logic in [`app/rehab/RehabPage.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/app/rehab/RehabPage.js).
- [`hooks/useIndexData.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/hooks/useIndexData.js): Tracker bootstrap state shell for exercises, programs, and logs. It composes the initial bootstrap workflow from [`lib/tracker-bootstrap.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/tracker-bootstrap.js), the older-history workflow from [`hooks/useIndexHistoryPagination.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/hooks/useIndexHistoryPagination.js), and sign-out cache reset behavior so the tracker route stays at orchestration level.
- [`hooks/useIndexHistoryPagination.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/hooks/useIndexHistoryPagination.js): Owns tracker older-history pagination beyond the initial bootstrap slice, including merged-history cache persistence and offline-aware failure copy.
- [`hooks/useUserContext.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/hooks/useUserContext.js): Shared resolver for profile-vs-auth identity domains. It now restores cached `users` context before attempting live refresh and keeps that cached profile/patient context authoritative while offline refreshes fail, so tracker, pt-view, and program keep their patient-scoped cache keys stable during offline use. It uses the shared effective-connectivity helper instead of relying only on raw browser online state. Tracker bootstrap and tracker log creation must use `patientId`/`profileId` from this hook where APIs expect `users.id`, not `session.user.id` (auth UUID).
...
- [`hooks/useProgramBootstrapWarmup.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/hooks/useProgramBootstrapWarmup.js): Shared protected-route warm hook for `/program` cold-open offline support. It runs from the resolved `useAuth()` browser session, uses the shared effective-connectivity helper, and fills missing editor bootstrap caches (users, exercises, programs, vocabularies, reference data) after authenticated app use so PT-office offline first visits to `/program` do not depend on a prior online `/program` load. It does not issue hidden credentialed document or RSC fetches; route warming stays on the documented prefetch path in `ProtectedClientWarmers`.
- [`hooks/useProgramPageData.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/hooks/useProgramPageData.js): Owns the `/program` bootstrap and cached fallback lifecycle, including therapist/admin access gating, patient-scoped context resolution, immediate cached editor bootstrap when IndexedDB already has data, server-seeded or `auth_state`-seeded cached restore before browser session hydration finishes, background refresh of the full editor payload, and route-level fallback copy when the shared offline-aware read path fails. Pure cached-snapshot shaping for this route lives in [`lib/program-page-data.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/program-page-data.js).
...
- [`hooks/useManualLog.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/hooks/useManualLog.js): Owns manual-log workflow orchestration on the tracker page: open/close behavior, coordination with draft-session state, and submit-time handoff back into the active tracker session. Pure manual-log set shaping now lives in [`lib/manual-log-state.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/lib/manual-log-state.js).
- [`hooks/useTrackerExerciseSessionState.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/hooks/useTrackerExerciseSessionState.js): Owns selected-exercise, draft-session, and timer-open lifecycle for the active tracker flow, including selection refresh and side continuity for sided exercises.
- [`hooks/useTrackerPendingSetFlow.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/hooks/useTrackerPendingSetFlow.js): Owns pending-set confirmation, edit, manual fallback, and undo behavior for the active tracker session.
- [`hooks/useTrackerSession.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/hooks/useTrackerSession.js): Owns top-level tracker-session orchestration by wiring the selected-exercise lifecycle, pending-set flow, and finalization lifecycle together for the tracker route.
...
- [`hooks/useTrackerSessionLifecycle.js`](C:/Users/cindi/OneDrive/Documents/GitHub/pttracker/hooks/useTrackerSessionLifecycle.js): Owns the finalization-hook integration and optimistic-log merge used by the active tracker session so `useTrackerSession.js` can stay at orchestration level.
