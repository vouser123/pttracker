// lib/dosage-summary.js — shared prescribed-dosage summary wording
export function formatDosageSummary(source, options = {}) {
  ...
  const sets = firstDefined(
    source?.current_sets,
    source?.sets,
    exercise?.current_sets,
    exercise?.sets,
    0,
  );
  const reps = firstDefined(
    source?.current_reps,
    source?.reps_per_set,
  ...
  const durationSeconds = firstDefined(
    source?.seconds_per_set,
    source?.seconds_per_rep,
    exercise?.seconds_per_set,
    exercise?.seconds_per_rep,
    0,
  );
  ...
  const modifiers =
    Array.isArray(source?.pattern_modifiers) && source.pattern_modifiers.length > 0
      ? source.pattern_modifiers
      : Array.isArray(exercise?.pattern_modifiers)
        ? exercise.pattern_modifiers
        : [];
