// lib/program-offline.js — offline queue helpers for program editor mutations and replay.

import { offlineCache } from './offline-cache';
import {
  addRole,
  batchAssignPrograms,
  batchUpdateProgramDates,
...
export function createProgramMutation(type, payload, extra = {}) {
  return {
    id:
      ...
      `program-mutation-${Date.now()}-${Math.random().toString(16).slice(2)}`,
    type,
    payload,
    status: 'pending',
    last_error: null,
...
export function markProgramMutationFailed(queue, mutationId, message) {
  return (queue ?? []).map((item) =>
    item.id === mutationId ? { ...item, status: 'failed', last_error: message } : item,
  );
}
...
export function mergeProgramMutationQueue(queue, mutation) {
  ...
  if (mutation.type === 'exercise.create') {
    const existingIndex = nextQueue.findIndex(
      (item) =>
        item.type === 'exercise.create' && item.payload.exerciseId === mutation.payload.exerciseId,
    );
    if (existingIndex >= 0) {
  ...
  if (mutation.type === 'exercise.update') {
    ...
    const updateIndex = nextQueue.findIndex(
      (item) =>
        item.type === 'exercise.update' && item.payload.exerciseId === mutation.payload.exerciseId,
    );
    if (updateIndex >= 0) {
  ...
  if (mutation.type === 'program.upsert') {
    const programIndex = nextQueue.findIndex(
      (item) =>
        item.type === 'program.upsert' && item.payload.exercise_id === mutation.payload.exercise_id,
    );
    if (programIndex >= 0) {
  ...
  if (mutation.type === 'program.batch.status' || mutation.type === 'program.batch.dates') {
    const batchIndex = nextQueue.findIndex(
      (item) =>
        item.type === mutation.type && item.payload.patient_id === mutation.payload.patient_id,
    );
    if (batchIndex >= 0) {
      return replaceAt(nextQueue, batchIndex, mutation);
    }
  ...
  if (
    mutation.type === 'vocab.create' ||
    mutation.type === 'vocab.update' ||
    mutation.type === 'vocab.delete'
  ) {
    const vocabIndex = nextQueue.findIndex(
      (item) =>
  ...
  if (mutation.type === 'role.delete' && isLocalRoleId(mutation.payload.roleId)) {
    return nextQueue.filter(
      (item) => !(item.type === 'role.add' && item.payload.localRoleId === mutation.payload.roleId),
    );
  }
...
export function getProgramMutationLabel(mutation) {
  switch (mutation?.type) {
    ...
    case 'program.upsert':
      return 'dosage';
    case 'program.batch.assign':
      return 'program assignment';
    case 'program.batch.status':
      return 'assignment status';
    case 'program.batch.dates':
      return 'assignment dates';
    case 'vocab.create':
    case 'vocab.update':
    case 'vocab.delete':
  ...
  switch (mutation.type) {
      ...
      return {
        ...
        payload: {
          ...
          payload: {
            ...
            id: resolveExerciseId(
              exerciseIdMap,
              mutation.payload.payload?.id ?? mutation.payload.exerciseId,
            ),
          },
        },
      };
    ...
    case 'program.batch.assign':
      return {
        ...
        payload: {
          ...mutation.payload,
          assignments: (mutation.payload.assignments ?? []).map((assignment) => ({
            ...assignment,
            exercise_id: resolveExerciseId(exerciseIdMap, assignment.exercise_id),
          })),
        },
      };
    default:
  ...
  switch (resolvedMutation.type) {
    ...
    case 'exercise.update':
      return updateExercise(
        accessToken,
        resolvedMutation.payload.exerciseId,
        resolvedMutation.payload.payload,
      );
    case 'role.add':
    ...
    case 'program.upsert':
      if (
        resolvedMutation.payload.programId &&
        !isLocalProgramId(resolvedMutation.payload.programId)
      ) {
        try {
    ...
    case 'program.batch.assign':
      return batchAssignPrograms(accessToken, resolvedMutation.payload);
    case 'program.batch.status':
      return batchUpdateProgramStatus(accessToken, resolvedMutation.payload);
    case 'program.batch.dates':
      return batchUpdateProgramDates(accessToken, resolvedMutation.payload);
    case 'vocab.create':
    ...
    try {
      ...
      if (mutation.type === 'exercise.create') {
        ...
        const createdExerciseId =
          result?.exercise?.id ?? mutation.payload.payload?.id ?? sourceExerciseId;
