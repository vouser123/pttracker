...
export function getDurationCompletionSpeech(sessionProgress, isSided, selectedSide) {
  const targetSets = Number(sessionProgress?.targetSets ?? 0) || 0;
  if (targetSets <= 0) return 'Set complete';

  if (isSided && selectedSide) {
    ...
    return `Set ${completedSetNumber} of ${targetSets} complete for ${selectedSide} side`;
  }
