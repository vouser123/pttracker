...
export function toDisplayValue(param) {
...
function getSessionSets(sessionSets, side) {
  return [...(sessionSets ?? [])].filter((set) => matchesSide(set, side)).reverse();
}
...
export function collectGlobalParameterValues(logs) {
  ...
  return Object.fromEntries(
    Object.entries(byParam).map(([name, values]) => [
      name,
...
export function collectAvailableParameterValues(logs, paramNames = [], options = {}) {
  const { side = null, sessionSets = [] } = options;
  const names = new Set((paramNames ?? []).map(normalizeString).filter(Boolean));
  const globalValues = collectGlobalParameterValues(logs);
  const byParam = {};

  function ensureParam(name) {
    if (!name || (names.size > 0 && !names.has(name))) return null;
    if (!byParam[name]) {
      byParam[name] = new Set(globalValues[name] ?? []);
    }
    return byParam[name];
  }
  ...
  for (const set of getSessionSets(sessionSets, side)) {
    for (const param of set?.form_data ?? []) {
      const name = normalizeString(param?.parameter_name);
      const bucket = ensureParam(name);
      if (!bucket) continue;
      const displayValue = toDisplayValue(param);
      if (displayValue) bucket.add(displayValue);
    }
  }

  if (names.size > 0) {
    for (const name of names) ensureParam(name);
  }
