...
export async function fetchUsers(token) {
  const data = await fetchJsonWithOffline('/api/users', {
    token,
    errorPrefix: 'fetchUsers failed',
    offlineMessage: 'Offline - users unavailable.',
  });
  return data.users ?? [];
}
 ...
 */
export function resolvePatientScopedUserContext(users, authUserId, patientId = null) {
  ...
  if (currentUser.role === 'therapist') {
    ...
    patientUser = patientId
      ? (patients.find((user) => user.id === patientId) ?? null)
      : (patients[0] ?? null);
...
export function deriveUserContextState(users, authUserId) {
  ...
  let otherUser = null;
  if (currentUser.role === 'therapist') {
    otherUser = users.find((user) => user.therapist_id === currentUser.id) ?? null;
  } else {
    otherUser = users.find((user) => user.id === currentUser.therapist_id) ?? null;
  }
