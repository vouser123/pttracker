// lib/exercise-sort.js — pure tracker exercise ordering and assignment-aware lifecycle filtering helpers
...
const LIFECYCLE_FILTERS = new Set(['routine', 'prn', 'all']);
...
export function normalizeLifecycleFilter(value) {
  return LIFECYCLE_FILTERS.has(value) ? value : 'routine';
}

export function getTrackerLifecycleFilterStorageKey(userId) {
  return userId ? `pt_tracker_lifecycle_filter_${userId}` : null;
}
...
export function applyManualOrder(exercises = [], manualOrderIds = []) {
  ...
  return normalizedIds.map((id) => exerciseMap.get(id)).filter(Boolean);
}
...
export function sortExercises({
  ...
  const ordered =
    normalizedMode === 'manual' ? applyManualOrder(exercises, manualOrderIds) : [...exercises];
  ...
  return ordered.filter((exercise) => {
    const resolvedStatus = resolveStatus
      ? resolveStatus(exercise, programsByExercise.get(exercise.id) ?? null)
      : null;
    const isArchived = resolvedStatus === 'archived' || isExerciseArchived(exercise);
    const isDeprecated = resolvedStatus === 'deprecated' || isExerciseDeprecated(exercise);
    const isRoutine = resolvedStatus ? resolvedStatus === 'active' : isExerciseRoutine(exercise);
    const isPrn = resolvedStatus ? resolvedStatus === 'as_needed' : isExercisePrn(exercise);

    if (isArchived || isDeprecated) return false;
    if (lifecycleFilter === 'routine' && !isRoutine) return false;
    if (lifecycleFilter === 'prn' && !isPrn) return false;
    if (lifecycleFilter === 'all' && !isRoutine && !isPrn) return false;
    if (!q) return true;
