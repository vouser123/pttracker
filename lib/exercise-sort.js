...
export function sortExercises({
    ...
    return ordered.filter((exercise) => {
        if (isExerciseArchived(exercise) || isExerciseDeprecated(exercise)) return false;
        if (lifecycleFilter === 'routine' && !isExerciseRoutine(exercise)) return false;
        if (lifecycleFilter === 'prn' && !isExercisePrn(exercise)) return false;
        if (lifecycleFilter === 'all' && !isExerciseRoutine(exercise) && !isExercisePrn(exercise)) return false;
        if (!q) return true;
