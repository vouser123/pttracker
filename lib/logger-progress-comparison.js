...
function isComparableSessionComplete(exercise, currentSets) {
  const targetSets = Math.max(0, Number(exercise?.current_sets ?? exercise?.sets ?? 0) || 0);
  if (!targetSets) return false;
  return currentSets.length >= targetSets;
}

export function getProgressComparison(
  logs,
  exercise,
  currentSessionSets,
  selectedSide = null,
  ...
  if (!isComparableSessionComplete(exercise, currentSets)) {
    return null;
  }
