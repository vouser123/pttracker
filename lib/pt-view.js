...
import { resolveEffectiveStatus } from './program-status.js';
...
export function isRoutineExercise(exercise, assignmentStatus = null) {
  return (
    resolveEffectiveStatus(getExerciseLifecycleStatus(exercise), assignmentStatus) === 'active'
  );
}
...
export function findNeedsAttention(logs, programs) {
  ...
  for (const program of programs) {
    if (!isRoutineExercise(program.exercises, program.assignment_status)) continue;
