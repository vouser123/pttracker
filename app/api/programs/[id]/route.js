...
import { resolveProgramDosageType, dosageTypeRequiresReps } from '../../../../lib/programs-utils.js';

/**
 * PUT /api/programs/:id — Update program dosage fields.
 * Body: { sets?, reps_per_set?, seconds_per_rep?, seconds_per_set?, distance_feet?, dosage_type? }
 */
export async function PUT(request, { params }) {
    ...
    const { sets, reps_per_set, seconds_per_rep, seconds_per_set, distance_feet, dosage_type } = body;
    ...
    try {
        ...
        if (fetchError || !existingProgram) return Response.json({ error: 'Program not found' }, { status: 404 });

        if (user.role === 'patient') {
            const isOwnAccount = user.id === existingProgram.patient_id || user.auth_id === existingProgram.patient_id;
        ...
        if (reps_per_set !== undefined) {
            if (reps_per_set === null) {
                if (dosageTypeRequiresReps(resolvedDosageType)) return badRequest('reps_per_set must be a positive integer');
        ...
        if (hasSecondsPerRep) {
            if (seconds_per_rep !== null && (!Number.isInteger(seconds_per_rep) || seconds_per_rep < 0)) return badRequest('seconds_per_rep must be a non-negative integer or null');
            updateData.seconds_per_rep = seconds_per_rep;
        }
        if (hasSecondsPerSet) {
            if (seconds_per_set !== null && (!Number.isInteger(seconds_per_set) || seconds_per_set < 0)) return badRequest('seconds_per_set must be a non-negative integer or null');
            updateData.seconds_per_set = seconds_per_set;
        }
        if (distance_feet !== undefined) {
            if (distance_feet !== null && (!Number.isInteger(distance_feet) || distance_feet < 1)) return badRequest('distance_feet must be a positive integer or null');
