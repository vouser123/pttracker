...
export async function GET(request) {
  ...
  const recordLimit = parseInt(limit, 10) || 300;
  const dayRange = parseInt(days, 10) || 14;
  ...
  try {
    ...
    if (beforeCursor && !Number.isNaN(beforeCursor.getTime())) {
    ...
    let formDataBySet = {};
    if (setIds.length > 0) {
      const formDataRows = await batchedIn(
        supabase,
...
export async function POST(request) {
  ...
  if (patient_id && patient_id !== user.id) {
    if (user.role !== 'therapist' && user.role !== 'admin') {
      return Response.json(
        { error: 'Only therapists or admins may log on behalf of a patient' },
        { status: 403 },
      );
    }
    ...
    if (user.role === 'therapist') {
      ...
      if (patientError || !patientRecord) {
        return Response.json(
          { error: 'Patient is not assigned to this therapist' },
          { status: 403 },
        );
      }
    }
  ...
  if (
    ...
    return badRequest(
      'Missing required fields: exercise_name, activity_type, performed_at, client_mutation_id, sets',
    );
  }
  ...
  try {
    ...
    const [{ data: log, error: logFetchError }, { data: createdSets, error: setsFetchError }] =
      await Promise.all([
  ...
  try {
    ...
    if (sets && Array.isArray(sets)) {
      ...
      for (let i = 0; i < sets.length; i++) {
        ...
        if (!createdSet) {
          throw new Error(
            `No created set found for set_number ${resolvedSetNumber} — clinical data integrity error`,
          );
        }
