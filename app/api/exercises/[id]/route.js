...
function validateExerciseData(data, isUpdate = false, validPtCategories = []) {
  ...
  if (!isUpdate) {
    ...
    if (
      ...
      data.canonical_name.trim().length === 0
    )
  ...
  if (data.equipment && typeof data.equipment !== 'object')
    errors.push('equipment must be an object');
  ...
  if (data.guidance && typeof data.guidance === 'object') {
    for (const [section, items] of Object.entries(data.guidance)) {
      if (!VALID_GUIDANCE_SECTIONS.includes(section))
        errors.push(`Invalid guidance section: ${section}`);
      if (Array.isArray(items)) {
        items.forEach((item, idx) => {
          if (typeof item !== 'string') errors.push(`guidance.${section}[${idx}] must be a string`);
            ...
            errors.push(`guidance.${section}[${idx}] exceeds ${MAX_GUIDANCE_ITEM_LENGTH} chars`);
        });
      }
    }
  }
...
export async function PUT(request, { params }) {
  ...
  try {
    ...
    if (lifecycle_effective_start_date !== undefined)
      updates.lifecycle_effective_start_date = lifecycle_effective_start_date;
    if (lifecycle_effective_end_date !== undefined)
      updates.lifecycle_effective_end_date = lifecycle_effective_end_date;
    if (supersedes_exercise_id !== undefined)
      updates.supersedes_exercise_id = supersedes_exercise_id || null;
      ...
      if (oldSupersedes !== newSupersedes) {
        if (oldSupersedes) {
          await supabase
            .from('exercises')
            .update({
              superseded_by_exercise_id: null,
    ...
    if (pattern_modifiers !== undefined) {
      ...
      if (pattern_modifiers.length > 0) {
        const { error: modError } = await supabase
          ...
          .insert(pattern_modifiers.map((modifier) => ({ exercise_id: exerciseId, modifier })));
    ...
    if (primary_muscles !== undefined || secondary_muscles !== undefined) {
      ...
      const muscleRows = [
        ...(primary_muscles || []).map((name) => ({
          exercise_id: exerciseId,
          muscle_name: name,
          is_primary: true,
        })),
        ...(secondary_muscles || []).map((name) => ({
          exercise_id: exerciseId,
          muscle_name: name,
          is_primary: false,
        })),
    ...
    if (form_parameters_required !== undefined) {
      ...
      if (form_parameters_required.length > 0) {
        const { error: paramError } = await supabase.from('exercise_form_parameters').insert(
          form_parameters_required.map((name) => ({
            exercise_id: exerciseId,
            parameter_name: name,
          })),
        );
    ...
    if (guidance !== undefined) {
      ...
      for (const [section, items] of Object.entries(guidance)) {
        items.forEach((content, index) => {
          guidanceRows.push({ exercise_id: exerciseId, section, content, sort_order: index });
        });
      }
