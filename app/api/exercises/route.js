...
function validateExerciseData(data, isUpdate = false, validPtCategories = []) {
  ...
  if (data.pattern_modifiers) {
    if (!Array.isArray(data.pattern_modifiers)) {
      ...
      if (invalid.length > 0)
        errors.push(
          `Invalid pattern modifiers: ${invalid.join(', ')}. Must be one of: ${VALID_MODIFIERS.join(', ')}`,
        );
    }
  }
    ...
    if (typeof data.guidance !== 'object') {
      ...
      for (const [section, items] of Object.entries(data.guidance)) {
        if (!VALID_GUIDANCE_SECTIONS.includes(section))
          errors.push(
            `Invalid guidance section: ${section}. Must be one of: ${VALID_GUIDANCE_SECTIONS.join(', ')}`,
          );
        if (!Array.isArray(items)) {
          ...
          items.forEach((item, idx) => {
            if (typeof item !== 'string')
              ...
              errors.push(
                `guidance.${section}[${idx}] exceeds ${MAX_GUIDANCE_ITEM_LENGTH} character limit`,
              );
          });
        }
      }
  ...
  try {
    ...
    if (pattern_modifiers.length > 0) {
      const { error: modError } = await supabase
        ...
        .insert(pattern_modifiers.map((modifier) => ({ exercise_id: id, modifier })));
    ...
    if (form_parameters_required.length > 0) {
      const { error: paramError } = await supabase
        ...
        .insert(
          form_parameters_required.map((name) => ({ exercise_id: id, parameter_name: name })),
        );
    ...
    for (const [section, items] of Object.entries(guidance)) {
      items.forEach((content, index) => {
        guidanceRows.push({ exercise_id: id, section, content, sort_order: index });
      });
    }
