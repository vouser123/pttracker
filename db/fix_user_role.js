/**
 * Fix user role - change cindi@example.invalid from patient to therapist
 */
...
async function fixUserRole() {
  console.log('Updating user role for cindi@example.invalid...\n');

  const { data: user, error: findError } = await supabase
    .from('users')
    .select('*')
    .eq('email', 'cindi@example.invalid')
    .single();
